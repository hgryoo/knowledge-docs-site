...
| 트랙 | 톤 | 어디서 영감을 얻나 |
|---|---|---|
| **slide** | 발표 데크와 동일 — 탱그램 무지개 띠, 남색 h1 + 빨강 밑줄, 주황 h2 좌측 바, 자홍 h3, 파랑 좌측 바 코드 블록, 호박 인용구 | `knowledge-slides/themes/cubrid.css` (분석서 데크 6편이 쓰는 테마) |
...
├── cubrid-lock-manager.slide.pdf     ← 새로 생성
...
PDF 가 md 옆에 존재하면 그 페이지의 H1 아래에 자동으로 ⇣ `PDF · slide-tone` / ⇣ `PDF · doc-tone` 버튼이 렌더된다. 메커니즘:
...
4. 버튼은 디렉터리 내 PDF 파일명 `<slug>.<track>.pdf` 의 `<track>` 부분을 라벨로 사용. `slide` / `doc` 는 친화적 라벨로 매핑, 그 외 트랙은 `PDF · <track>` 형식.
...
두 트랙의 차이는 `pdf-export/styles/print-slide.css` vs `print-doc.css` 단 두 파일이다. 둘 다:
...
├── styles/
│   ├── print-slide.css      # Track A — slide-tone (Track 추가 시 여기에 한 줄)
│   └── print-doc.css        # Track B — doc-tone
├── assets/
│   ├── cubrid-logo.png      # 700px transparent (예비)
│   └── cubrid-logo-small.png# 193x90, 푸터에 임베드 (slide track)
